...
echo "======================================"
#Installation DB
docker run --name rancher-app-$VERSION_RANCHER --restart=always -d -v /var/docker/volume/rancher-db:/var/lib/mysql -v /var/docker/logs/rancher-db:/var/log/mysql -e MYSQL_USER="$DB_USER" -e MYSQL_PASSWORD="$DB_PASSWORD" -e MYSQL_ROOT_PASSWORD="$DB_ROOT_PWD" -e MYSQL_DATABASE="rancher" -v /var/docker/logs/rancher-app:/var/lib/cattle/logs -v /etc/ssl:/etc/ssl -e CATTLE_DB_CATTLE_MYSQL_PORT=3306 -e CATTLE_DB_CATTLE_MYSQL_NAME="rancher" -e CATTLE_DB_CATTLE_USERNAME="$DB_USER" -e CATTLE_DB_CATTLE_PASSWORD="$DB_PASSWORD" -p 9080:8080 rancher/server:$VERSION_RANCHER
